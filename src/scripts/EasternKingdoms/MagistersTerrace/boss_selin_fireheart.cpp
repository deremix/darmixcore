    ...
    boss_selin_fireheartAI(Creature* c) : ScriptedAI(c)
    {
        ...
        if (pInstance)
        {
            ...
            for (uint8 i = 0; i < size; ++i)
            {
                ...
                debug_log("DSCR: Selin: Adding Fel Crystal %llu to list", guid);
    ...
    void MovementInform(uint32 type, uint32 id)
    {
        if (type == POINT_MOTION_TYPE && id == 1)
        {
            ...
            else
            {
                ...
                error_log("DSCR: Selin Fireheart unable to drain crystal as the crystal is either dead or despawned");
                DrainingCrystal = false;
            }
        }
    }
