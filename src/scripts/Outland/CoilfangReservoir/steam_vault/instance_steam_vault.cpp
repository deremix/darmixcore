...
struct instance_steam_vault : public ScriptedInstance
{
    ...
    Player* GetPlayerInMap()
    {
        ...
        debug_log("DSCR: Instance Steamvault: GetPlayerInMap, but PlayerList is empty!");
        return NULL;
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        if (!player)
        {
            debug_log("DSCR: Instance Steamvault: SetData (Type: %u Data %u) cannot find any player.", type, data);
            return;
        }
        ...
        switch (type)
        {
            case TYPE_HYDROMANCER_THESPIA:
                if (data == SPECIAL)
                {
                    ...
                    debug_log("DSCR: Instance Steamvault: Access panel used.");
                }
                Encounter[0] = data;
                break;
