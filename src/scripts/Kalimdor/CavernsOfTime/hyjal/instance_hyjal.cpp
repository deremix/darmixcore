...
struct instance_mount_hyjal : public ScriptedInstance
{
    ...
    void SetData(uint32 type, uint32 data)
    {
        switch (type)
        {
         ...
         debug_log("DSCR: Instance Hyjal: Instance data updated for event %u (Data=%u)",type,data);
    ...
    void UpdateWorldState(uint32 id, uint32 state)
    {
        ...
        } else debug_log("DSCR: Instance Hyjal: UpdateWorldState, but PlayerList is empty!");
    }
