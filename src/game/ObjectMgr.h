...
#define MIN_DARMIX_STRING_ID           1                    // 'darmix_string'
#define MAX_DARMIX_STRING_ID           2000000000
#define MIN_DB_SCRIPT_STRING_ID        MAX_DARMIX_STRING_ID // 'db_script_string'
...
class ObjectMgr
{
    ...
    public:
      ...
      //Player* GetPlayer(uint64 guid) const { return ObjectAccessor::FindPlayer(guid); }                      // removed
        ...
        bool LoaddarmixStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value);
        bool LoaddarmixStrings() { return LoaddarmixStrings(WorldDatabase,"darmix_string",MIN_DARMIX_STRING_ID,MAX_DARMIX_STRING_ID); }
...
bool LoaddarmixStrings(DatabaseType& db, char const* table,int32 start_value = MAX_CREATURE_AI_TEXT_STRING_ID, int32 end_value = std::numeric_limits<int32>::min());
uint32 GetAreaTriggerScriptId(uint32 trigger_id);
uint32 GetScriptId(const char *name);
