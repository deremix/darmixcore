...
bool ObjectMgr::LoaddarmixStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value)
{
    ...
    if (!result)
    {
        ...
        sLog.outString();
        if (min_value == MIN_DARMIX_STRING_ID)              // error only in case internal strings
            sLog.outErrorDb(">> Loaded 0 darmix strings. DB table %s is empty. Cannot continue.",table);
        else
    ...
    sLog.outString();
    if (min_value == MIN_DARMIX_STRING_ID)
        sLog.outString(">> Loaded %u darmix strings from table %s", count,table);
    else
...
const char *ObjectMgr::GetBlizzLikeString(int32 entry, int locale_idx) const
{
    ...
    if (entry > 0)
        sLog.outErrorDb("Entry %i not found in darmix_string table.",entry);
    else
        sLog.outErrorDb("darmix string entry %i not found in DB.",entry);
    return "<error>";
}
...
{
    LoaddarmixStrings(WorldDatabase,"db_script_string",MIN_DB_SCRIPT_STRING_ID,MAX_DB_SCRIPT_STRING_ID);
...
bool LoaddarmixStrings(DatabaseType& db, char const* table,int32 start_value, int32 end_value)
{
    ...
    return objmgr.LoaddarmixStrings(db,table,start_value,end_value);
}
