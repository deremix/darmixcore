...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        debug_log("DSCR: EscortAI has left combat and is now returning to last point");
    }
    else
    {
...
void npc_escortAI::UpdateAI(const uint32 uiDiff)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && !me->getVictim() && m_uiWPWaitTimer && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiWPWaitTimer <= uiDiff)
        {
            ...
            if (CurrentWP == WaypointList.end())
            {
                if (DespawnAtEnd)
                {
                    debug_log("DSCR: EscortAI reached end of waypoints");
                    ...
                    if (m_bCanReturnToStart)
                    {
                        ...
                        debug_log("DSCR: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
                        return;
                    }
                ...
                {
                    debug_log("DSCR: EscortAI reached end of waypoints with Despawn off");
            ...
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                debug_log("DSCR: EscortAI start waypoint %u (%f, %f, %f).", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_uiPlayerGUID && !me->getVictim() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiPlayerCheckTimer <= uiDiff)
        {
            ...
            {
                debug_log("DSCR: EscortAI failed because player/group was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 uiMoveType, uint32 uiPointId)
{
    ...
    {
        debug_log("DSCR: EscortAI has returned to original position before combat");
    ...
    {
        debug_log("DSCR: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
    ...
    else
    {
        ...
        debug_log("DSCR: EscortAI Waypoint %u reached", CurrentWP->id);
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!m_bIsRunning)
            ...
            debug_log("DSCR: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            debug_log("DSCR: EscortAI attempt to set walk mode, but is already walking.");
    }
    m_bIsRunning = bRun;
}
...
void npc_escortAI::Start(bool bIsActiveAttacker, bool bRun, uint64 uiPlayerGUID, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        debug_log("DSCR: EscortAI attempt to Start while already escorting.");
        return;
    }
    ...
    if (WaypointList.empty())
    {
        error_db_log("DSCR: EscortAI Start with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).", pQuest ? pQuest->GetQuestId() : 0);
        return;
    }
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        debug_log("DSCR: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        me->GetMotionMaster()->MovementExpired();
        me->GetMotionMaster()->MoveIdle();
        debug_log("DSCR: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }

    //disable npcflags
    me->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);

    debug_log("DSCR: EscortAI started with %u waypoints. ActiveAttacker = %d, Run = %d, PlayerGUID = %llu", WaypointList.size(), m_bIsActiveAttacker, m_bIsRunning, m_uiPlayerGUID);
