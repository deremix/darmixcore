...
void ScriptedAI::DoResetThreat()
{
    ...
    {
        error_log("DSCR: DoResetThreat called for creature that either cannot have threat list or has empty threat list (me entry = %d)", me->GetEntry());
        return;
    }
...
void ScriptedAI::DoTeleportPlayer(Unit* pUnit, float fX, float fY, float fZ, float fO)
{
    ...
    {
        if (pUnit)
            error_log("DSCR: Creature %llu (Entry: %u) Tried to teleport non-player unit (Type: %u GUID: %u) to x: %f y:%f z: %f o: %f. Aborted.", me->GetGUID(), me->GetEntry(), pUnit->GetTypeId(), pUnit->GetGUID(), fX, fY, fZ, fO);
        return;
    }
...
bool ScriptedAI::EnterEvadeIfOutOfCombatArea(const uint32 uiDiff)
{
    ...
    switch (me->GetEntry())
    {
        ...
        case NPC_SARTHARION:                                         // sartharion (calculate box)
            ...
            break;
        default:
            error_log("DSCR: EnterEvadeIfOutOfCombatArea used for creature entry %u, but does not have any definition.", me->GetEntry());
            return false;
    }
